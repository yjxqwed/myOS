...
static void partition_scan(uint8_t drive, uint32_t base_lba) {
    IDE_DEVICE *hd = &(g_ide_devices[drive]);
    if (IDE_ATA != hd->type) {
        return;
    }
    ...
    ide_read_sectors(drive, 1, base_lba, bs);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (pe->fs_type == 0) {
            ...
            if (primary_ext_base_lba == 0) {
                ...
                partition_scan(drive, pe->start_lba);
            } else {
                partition_scan(drive, primary_ext_base_lba + pe->start_lba);
            }
        } else {
...
static void print_partitions() {
    ...
    __list_for_each((&partition_list), p) {
        PARTITION *part = __container_of(PARTITION, part_tag, p);
        kprintf(
            KPL_NOTICE,
            "    %x start_lba: 0x%X, sec_cnt: 0x%X\n",
            part->my_drive, part->start_lba, part->sec_cnt
...
int ata_read(uint8_t drive, uint32_t lba, uint32_t buffer, uint8_t sec_cnt) {
    return ide_read_sectors(drive, sec_cnt, lba, buffer);
}

int ata_write(uint8_t drive, uint32_t lba, uint32_t buffer, uint8_t sec_cnt) {
    return ide_write_sectors(drive, sec_cnt, lba, buffer);
}
